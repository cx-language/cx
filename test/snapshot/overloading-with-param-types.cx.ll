...
define i32 @main() {
  ...
  %s.load = load %S, %S* %s, align 4
  call void @_EN4main1fE1S(%S %s.load)
  %t.load = load %T, %T* %t, align 4
  call void @_EN4main1fE1T(%T %t.load)
  %s.load1 = load %S, %S* %s, align 4
  %s.load2 = load %S, %S* %s, align 4
  call void @_EN4mainplE1S1S(%S %s.load1, %S %s.load2)
  %t.load3 = load %T, %T* %t, align 4
  %t.load4 = load %T, %T* %t, align 4
