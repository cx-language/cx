...
define i32 @main() {
  ...
  %1 = alloca %vec2
  %2 = alloca %vec2
  call void @_EN4main4vec24initE1x3int1y3int(%vec2* %v, i32 3, i32 2)
  call void @_EN4main4vec24initE1x3int1y3int(%vec2* %1, i32 2, i32 4)
  %.load = load %vec2, %vec2* %1
  %v.load = load %vec2, %vec2* %v
  %3 = call %vec2 @_EN4mainmlE1a4vec21b4vec2(%vec2 %.load, %vec2 %v.load)
  store %vec2 %3, %vec2* %v
  %v.load1 = load %vec2, %vec2* %v
  call void @_EN4main4vec24initE1x3int1y3int(%vec2* %2, i32 -1, i32 3)
  %.load2 = load %vec2, %vec2* %2
  %4 = call i1 @_EN4maineqE1a4vec21b4vec2(%vec2 %v.load1, %vec2 %.load2)
  %x = getelementptr inbounds %vec2, %vec2* %v, i32 0, i32 0
  %x.load = load i32, i32* %x
  %5 = call i32 @_EN4main4vec2ixE5index3int(%vec2* %v, i32 %x.load)
  ret i32 0
}
